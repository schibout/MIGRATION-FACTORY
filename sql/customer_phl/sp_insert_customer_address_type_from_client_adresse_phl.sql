CREATE OR REPLACE PROCEDURE clean_data.sp_insert_customer_address_type_from_client_adresse_phl()
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    v_start_time := NOW();
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Début insertion types d''adresse clients PHL - %', TO_CHAR(v_start_time, 'YYYY-MM-DD HH24:MI:SS');
    RAISE NOTICE '========================================';
    ...
    CALL clean_data.sp_insert_customer_address_type_single_phl('DELIVERY', v_inserted_count);
    ...
    CALL clean_data.sp_insert_customer_address_type_single_phl('INVOICE', v_inserted_count);
    v_total_inserted := v_total_inserted + v_inserted_count;
    ...
    CALL clean_data.sp_insert_customer_address_type_single_phl('DOCUMENT', v_inserted_count);
    v_total_inserted := v_total_inserted + v_inserted_count;
...
EXCEPTION
    WHEN OTHERS THEN
        v_end_time := NOW();
        RAISE EXCEPTION '❌ Erreur lors de l''INSERT types d''adresse depuis client_adresse_phl - %: %', 
