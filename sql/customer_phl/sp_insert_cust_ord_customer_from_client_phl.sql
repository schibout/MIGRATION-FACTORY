CREATE OR REPLACE PROCEDURE clean_data.sp_insert_cust_ord_customer_from_client_phl()
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    v_start_time := NOW();
    RAISE NOTICE 'Début insertion cust ord customer PHL - %', TO_CHAR(v_start_time, 'YYYY-MM-DD HH24:MI:SS');
    ...
    SELECT DISTINCT ON (cp.customer_id)
        cp.customer_id,                                                                  -- customer_no
        'STD'::VARCHAR(10),                                                              -- cust_grp
        0::NUMERIC(3,0),                                                                 -- cycle_period
        'EUR'::VARCHAR(3),                                                               -- currency_code
        'E'::VARCHAR(2),                                                                 -- category_db
        cp.customer_id,                                                                  -- customer_no_pay
        'STD'::VARCHAR(10),                                                              -- cust_price_group_id
        'DEFAULT'::VARCHAR(20),                                                          -- salesman_code
        'DEFAULT'::VARCHAR(10),                                                          -- market_code
        'STD'::VARCHAR(10),                                                              -- print_control_code
        'N'::VARCHAR(1),                                                                 -- cr_stop_db
        COALESCE(cp.name, '')::VARCHAR(100),                                             -- cust_ref
        NULL::DATE,                                                                      -- date_del
        'N'::VARCHAR(2),                                                                 -- invoice_sort_db
        NULL::DATE,                                                                      -- last_ivc_date
        SUBSTRING(COALESCE(cp.name, ''), 1, 2000),                                      -- note_text
        ...
        'Standard'::VARCHAR(25),                                                         -- discount_type
        ...
        '*'::VARCHAR(10),                                                                -- credit_control_group_id
        ...
        SUBSTRING(TRIM(COALESCE(cp.name, '')), 1, 100)                                  -- name
...
EXCEPTION
    WHEN OTHERS THEN
        v_end_time := NOW();
        RAISE EXCEPTION 'Erreur lors de l''INSERT cust ord customer depuis client_phl - %: %',
