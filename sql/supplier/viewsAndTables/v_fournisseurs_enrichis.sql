-- Vue enrichie des fournisseurs SAP
-- Sources: LFA1 (master), LFB1 (société), LFM1 (achats min ekorg), ADR6 (email)
--
-- Historique: INNER JOIN sur lfb1/lfm1 excluait tout fournisseur sans extension
-- comptable ou achat → vue vide si extraction partielle. Désormais LFA1 pilote
-- et lfb1/lfm1 sont en LEFT JOIN.
--
-- Si l'objet existe comme vue matérialisée, créer une vue normale sans drop préalable
-- provoque: ERROR: "v_fournisseurs_enrichis" is not a view
...
SELECT 
    ...
    GREATEST(COALESCE(a.updat, '19000101'::character varying), COALESCE(b.updat, '19000101'::character varying), COALESCE(m.erdat, '19000101'::character varying)) AS "Dernière modification SAP",
    CASE
        WHEN GREATEST(COALESCE(a.updat, '19000101'::character varying), COALESCE(b.updat, '19000101'::character varying), COALESCE(m.erdat, '19000101'::character varying))::text = COALESCE(a.updat, '19000101'::character varying)::text THEN COALESCE(a.uptim, ''::character varying)
        WHEN GREATEST(COALESCE(a.updat, '19000101'::character varying), COALESCE(b.updat, '19000101'::character varying), COALESCE(m.erdat, '19000101'::character varying))::text = COALESCE(b.updat, '19000101'::character varying)::text THEN COALESCE(b.uptim, ''::character varying)
...
GROUP BY a.lifnr, a.name1, a.name2, a.name3, a.name4, a.stras, a.ort01, a.ort02, a.pstlz, a.land1, a.regio, a.stcd1, a.stceg, a.stcd2, a.stcd3, a.telf1, a.telf2, a.telfx, adr6.smtp_addr, a.ktokk, a.brsch, a.spras, m.ekorg, m.ekgrp, m.sperm, m.plifz, m.minbw, m.zterm, m.inco1, m.inco2, m.verkf, m.zolla, a.erdat, a.ernam, a.updat, a.uptim, b.updat, b.uptim, m.erdat, m.ernam, a.loevm
ORDER BY a.lifnr;
