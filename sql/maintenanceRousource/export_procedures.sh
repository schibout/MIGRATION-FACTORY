...
# Script d'export des procédures stockées MAINTENANCE RESOURCE depuis la base PostgreSQL
...
# Début du script
echo "=========================================="
echo "  Export des procédures MAINTENANCE RESOURCE"
echo "=========================================="
echo ""
...
procedures=(
    "populate_ifs_person"
    "populate_resource_detail_file"
    "populate_resource_connection"
    "populate_resource_availability"
    "populate_resource_parent"
    "populate_maint_person_resource"
    "populate_all_tables"
...
output_files=(
    "01_populate_ifs_person.sql"
    "02_populate_resource_detail_file.sql"
    "03_populate_resource_connection.sql"
    "04_populate_resource_availability.sql"
    "05_populate_resource_parent.sql"
    "06_populate_maint_person_resource.sql"
    "07_populate_all_tables.sql"
    "08_get_resource_parents.sql"
)
...
# Fonction spéciale pour exporter find_person_id (du schéma raw_data)
export_find_person_id() {
    local output_file="00_find_person_id.sql"
    
    log_info "Export de find_person_id (raw_data)..."
    ...
    if [ -n "$result" ] && ! echo "$result" | grep -qi "error"; then
        {
            echo "-- ============================================================================"
            echo "-- Fonction utilitaire pour rechercher un person_id"
            echo "-- Schéma: raw_data"
            echo "-- ============================================================================"
            echo "$result;"
        } > "$output_file"
        ...
        log_warning "⚠ find_person_id non trouvée dans raw_data"
        return 1
    fi
}
...
# Export spécial pour find_person_id (du schéma raw_data)
if export_find_person_id; then
    ((count++))
else
    ((errors++))
fi
