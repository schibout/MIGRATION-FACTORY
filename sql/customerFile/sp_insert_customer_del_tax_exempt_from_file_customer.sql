...
BEGIN
    ...
    SELECT DISTINCT ON (fc.customer_id, fc.address_id, public.get_transcodification('COUNTRY', COALESCE(k.LAND1, fc.country, 'FR')), COALESCE(NULLIF(TRIM(fc.vat_number),''), NULLIF(TRIM(fc.tax_number_1),''), NULLIF(TRIM(fc.tax_number_2),''), 'NO_CERT'))
        ...
        public.get_default_value('clean_data.customer_del_tax_exempt', 'company') as COMPANY,
        ...
        public.get_default_value('clean_data.customer_del_tax_exempt', 'exempt_certificate_type_db') as EXEMPT_CERTIFICATE_TYPE_DB,
        public.get_default_value('clean_data.customer_del_tax_exempt', 'certificate_amount')::numeric as CERTIFICATE_AMOUNT
