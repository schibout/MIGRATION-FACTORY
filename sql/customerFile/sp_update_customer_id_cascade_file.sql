CREATE OR REPLACE PROCEDURE clean_data.sp_update_customer_id_cascade_file(IN p_old_customer_id character varying, IN p_new_customer_id character varying)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    v_start_time := NOW();
    RAISE NOTICE 'Début mise à jour customer_id FILE: % → % - %',
    ...
    v_end_time := NOW();
    RAISE NOTICE '✅ Mise à jour customer_id FILE terminée - %: % lignes au total mises à jour dans 20 tables',
...
EXCEPTION
    WHEN OTHERS THEN
        v_end_time := NOW();
        RAISE EXCEPTION '❌ Erreur lors de la mise à jour customer_id FILE - %: %',
