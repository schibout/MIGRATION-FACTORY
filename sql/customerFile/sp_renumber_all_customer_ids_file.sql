CREATE OR REPLACE PROCEDURE clean_data.sp_renumber_all_customer_ids_file(IN p_starting_id integer DEFAULT 700000)
 LANGUAGE plpgsql
AS $procedure$
DECLARE
...
BEGIN
    v_start_time := NOW();
    RAISE NOTICE '========================================';
    RAISE NOTICE 'Début renumérotation customer_id FILE - %', TO_CHAR(v_start_time, 'YYYY-MM-DD HH24:MI:SS');
    RAISE NOTICE 'ID de départ: %', p_starting_id;
    RAISE NOTICE '========================================';

    -- Compter le nombre de clients FILE à traiter
    SELECT COUNT(*) INTO v_customer_count
    FROM clean_data.customer_info;

    RAISE NOTICE 'Nombre de clients FILE à renuméroter: %', v_customer_count;

    -- Créer une table temporaire pour stocker le mapping des anciens et nouveaux IDs
    CREATE TEMP TABLE IF NOT EXISTS temp_customer_id_mapping_file (
        old_customer_id VARCHAR(50),
        new_customer_id VARCHAR(50),
        row_num INTEGER
    );

    TRUNCATE TABLE temp_customer_id_mapping_file;

    -- Remplir la table temporaire avec les anciens IDs et les nouveaux IDs séquentiels
    INSERT INTO temp_customer_id_mapping_file (old_customer_id, new_customer_id, row_num)
    SELECT
        customer_id,
    ...
    RAISE NOTICE 'Table temporaire de mapping créée avec % enregistrements',
        (SELECT COUNT(*) FROM temp_customer_id_mapping_file);
    ...
    v_end_time := NOW();
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ Renumérotation customer_id FILE terminée - %', TO_CHAR(v_end_time, 'YYYY-MM-DD HH24:MI:SS');
    ...
    DROP TABLE IF EXISTS temp_customer_id_mapping_file;
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        v_end_time := NOW();
        RAISE EXCEPTION '❌ Erreur lors de la renumérotation customer_id FILE - %: %',
