...
BEGIN
    ...
    SELECT DISTINCT
        ic.customer_number,                                                              -- customer_no
        SUBSTRING(COALESCE(ic.customer_group, 'STD'), 1, 10),                           -- cust_grp
        0::NUMERIC(3,0),                                                                 -- cycle_period
        SUBSTRING(COALESCE(MAX(knvv.WAERS), 'EUR'), 1, 3),                              -- currency_code
        'E'::VARCHAR(2),                                                                 -- category_db
        ic.customer_number,                                                              -- customer_no_pay
        SUBSTRING(COALESCE(MAX(knvv.PLTYP), 'STD'), 1, 10),                            -- cust_price_group_id
        SUBSTRING(COALESCE(MAX(knvv.VKGRP), 'DEFAULT'), 1, 20),                        -- salesman_code
        SUBSTRING(COALESCE(ic.sales_district, 'DEFAULT'), 1, 10),                      -- market_code
        'STD'::VARCHAR(10),                                                              -- print_control_code
        CASE WHEN ic.sales_order_block IS NOT NULL AND ic.sales_order_block != ''
             THEN 'Y' ELSE 'N' END::VARCHAR(1),                                         -- cr_stop_db
        SUBSTRING(COALESCE(ic.customer_classification, ''), 1, 100),                   -- cust_ref
        NULL::DATE,                                                                      -- date_del
        'N'::VARCHAR(2),                                                                 -- invoice_sort_db
        NULL::DATE,                                                                      -- last_ivc_date
        SUBSTRING(COALESCE(ic.name_1, ''), 1, 2000),                                   -- note_text
        ...
        SUBSTRING(TRIM(COALESCE(ic.name_1, '')), 1, 100)                                -- name
