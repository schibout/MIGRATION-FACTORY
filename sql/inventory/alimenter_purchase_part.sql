...
COMMENT ON FUNCTION clean_data.alimenter_purchase_part() IS
'Alimente la table PURCHASE_PART depuis ifs_article_maitre et tables SAP.
Colonnes alignées sur le fichier d''import IFS (paires code / code_db).
Règles de gestion Trimet:
- CONTRACT: 9200=SJ, 9000=CS
- PART_NO: numero_article SAP (pas de transcodification, l''article garde son ID)
- INVENTORY_FLAG: IBAU/DIEN/NLAG => No/N, sinon Yes/Y
- EXTERNAL_RESOURCE: DIEN => Yes/TRUE, sinon No/FALSE
- CLOSE_CODE: Automatic / Y
- PROCESS_TYPE: STD
- TAXABLE: Yes / TRUE
- COMPANY: TRIMET
- DATE_CRE: MARA.ERSDA
- Colonnes vides: ENG_ATTRIBUTE, NOTE_ID, QC_CODE, STAT_GRP, NOTE_TEXT, QC_DATE,
  BUYER_CODE, STD_NAME_DESCRIPTION, TECHNICAL_COORDINATOR_ID, STATISTICAL_CODE,
  STATISTICAL_CODE_MANUF, QUALITY_SYSTEM_LEVEL_ID, QSL_APPROVAL_TEMPLATE, QMR_APPROVAL_TEMPLATE';
