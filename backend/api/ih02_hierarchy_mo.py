...
Contrats JSON STRICTEMENT identiques a l'ancien blueprint (api/ih02_hierarchy.py)
-> le frontend fonctionne sans modification. Les identifiants exposes restent les
cles SAP (tplnr / equnr / idnrk) ; l'id bigint interne n'est jamais expose.

Bascule via le flag Config.IH02_USE_MAINTENANCE_OBJECT (voir api/__init__.py).
...
ih02_hierarchy_mo_blueprint = Blueprint('ih02_hierarchy_mo', __name__)
...
@ih02_hierarchy_mo_blueprint.route('/root-nodes', methods=['GET'])
def get_root_nodes():
    try:
        ...
        current_app.logger.error(f"Erreur racines IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/children', methods=['GET'])
def get_children():
    try:
        ...
        current_app.logger.error(f"Erreur enfants IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/node-details', methods=['GET'])
def get_node_details():
    try:
        ...
        current_app.logger.error(f"Erreur détails noeud IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/search', methods=['GET'])
def search_nodes():
    try:
...
@ih02_hierarchy_mo_blueprint.route('/update-location', methods=['PUT'])
def update_location():
    try:
        ...
        with get_db_connection() as conn:
            ...
            if 'code' in data and (data.get('code') or '').strip():
                ...
                if cursor.fetchone():
                    return jsonify({'success': False,
                                    'error': f'Identifiant "{new_code}" déjà utilisé par un autre poste technique'}), 409
            ...
            if 'parent_node_id' in data:
                ...
                new_pid = None
                if new_parent:
                    new_pid = _resolve_id(cursor, 'FUNC_LOC', new_parent)
                    if new_pid is None:
                        return jsonify({'success': False, 'error': f'Parent "{new_parent}" non trouvé'}), 404
                sets.append('parent_id = %s'); params.append(new_pid); modified.append('Parent')
        ...
        current_app.logger.error(f"Erreur update location IH02(MO): {e}")
        return jsonify({'success': False, 'error': f'Erreur serveur: {str(e)}'}), 500


@ih02_hierarchy_mo_blueprint.route('/stats', methods=['GET'])
def get_stats():
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
        ...
        current_app.logger.error(f"Erreur stats IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/export-structures', methods=['GET'])
def export_structures():
    try:
        ...
        current_app.logger.error(f"Erreur export structures IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/move-node', methods=['PUT'])
def move_node():
    try:
        data = request.get_json()
        tplnr = data.get('row_id') or data.get('node_id')
        ...
        current_app.logger.error(f"Erreur déplacement noeud IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/descendants-count', methods=['GET'])
def descendants_count():
    try:
        ...
        current_app.logger.error(f"Erreur comptage descendants IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/bulk-update', methods=['PUT'])
def bulk_update():
    ...
    try:
        ...
        current_app.logger.error(f"Erreur bulk-update IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/add-node', methods=['POST'])
def add_node():
    try:
        data = request.get_json()
        ...
        current_app.logger.error(f"Erreur ajout noeud IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/delete-node', methods=['DELETE'])
def delete_node():
    ...
    try:
        ...
        current_app.logger.error(f"Erreur suppression noeud IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
@ih02_hierarchy_mo_blueprint.route('/equipment-details', methods=['GET'])
def get_equipment_details():
    try:
        ...
        current_app.logger.error(f"Erreur détails équipement IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/equipment-children', methods=['GET'])
def get_equipment_children():
    try:
        ...
        current_app.logger.error(f"Erreur enfants équipement IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/update-equipment', methods=['PUT'])
def update_equipment():
    try:
        ...
        current_app.logger.error(f"Erreur update équipement IH02(MO): {e}")
        return jsonify({'success': False, 'error': f'Erreur serveur: {str(e)}'}), 500


@ih02_hierarchy_mo_blueprint.route('/add-equipment', methods=['POST'])
def add_equipment():
    try:
        data = request.get_json()
        ...
        current_app.logger.error(f"Erreur ajout équipement IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/delete-equipment', methods=['DELETE'])
def delete_equipment():
    try:
        ...
        current_app.logger.error(f"Erreur suppression équipement IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
@ih02_hierarchy_mo_blueprint.route('/bom/<path:tplnr>', methods=['GET'])
def get_fl_bom(tplnr):
    ...
    try:
        ...
        current_app.logger.error(f"Erreur BOM IH02(MO) ({tplnr}): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/bom-counts', methods=['GET'])
def get_fl_bom_counts():
    try:
        ...
        current_app.logger.error(f"Erreur BOM counts IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/article-bom/<path:matnr>', methods=['GET'])
def get_article_bom(matnr):
    ...
    try:
        ...
        current_app.logger.error(f"Erreur BOM article IH02(MO) ({matnr}): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/article-bom-counts', methods=['GET'])
def get_article_bom_counts():
    try:
        ...
        current_app.logger.error(f"Erreur counts BOM article IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
@ih02_hierarchy_mo_blueprint.route('/bom-component', methods=['PUT'])
def update_bom_component():
    ...
    try:
        ...
        current_app.logger.error(f'Erreur update BOM IH02(MO): {e}', exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/article-bom-component', methods=['PUT'])
def update_article_bom_component():
    ...
    try:
        ...
        current_app.logger.error(f'Erreur update BOM article IH02(MO): {e}', exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/article-master', methods=['PUT'])
def update_article_master():
    ...
    try:
        ...
        current_app.logger.error(f'Erreur renommage article IH02(MO): {e}', exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500
...
@ih02_hierarchy_mo_blueprint.route('/work-centers', methods=['GET'])
def get_work_centers():
    try:
        ...
        current_app.logger.error(f"Erreur postes de travail IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/search-equipment', methods=['GET'])
def search_equipment():
    ...
    try:
        ...
        current_app.logger.error(f"Erreur recherche équipement IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/search-article', methods=['GET'])
def search_article():
    ...
    try:
        ...
        current_app.logger.error(f"Erreur recherche article IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@ih02_hierarchy_mo_blueprint.route('/export-equipment', methods=['GET'])
def export_equipment():
    try:
        ...
        current_app.logger.error(f"Erreur export équipements IH02(MO): {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
