...
const ResourceDetailFilePageAdvanced = () => {
    const theme = useTheme();
    
    // Données principales
    const endpoint = 'detail-files';
    ...
    useEffect(() => {
        const loadTableData = async () => {
            setLoading(true);
            try {
                const data = await resourcesService.getTableData(
                    endpoint,
                    page,
                    rowsPerPage,
                    sortField,
    ...
    const handleExport = async (format: 'csv' | 'excel') => {
        ...
        try {
            ...
            const options: ExportOptions = {
                format,
                filename: `resource_detail_file_${new Date().toISOString().split('T')[0]}.${format === 'csv' ? 'csv' : 'xlsx'}`,
                includeHeaders: true,
                searchTerm,
                filters
            };
    ...
    const handleSaveEdit = async () => {
        try {
            ...
            const recordId = currentRow.resource_seq || currentRow.id || Object.values(currentRow)[0];
    ...
    const handleDeleteMultiple = async (rowsToDelete: any[]) => {
        ...
        for (const row of rowsToDelete) {
            try {
                const recordId = row.resource_seq || row.id || Object.values(row)[0];
                await resourcesService.deleteRecord(endpoint, recordId);
                successCount++;
            } catch (error) { /* ignore */ }
        }
        ...
        <Box sx={{ 
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                        ...
                        <Box>
                            ...
                            <Typography variant="body2" sx={{ color: 'rgba(255,255,255,0.7)', mt: 0.5 }}>
                                Gestion des fichiers de détails des ressources
                            </Typography>
                        </Box>
                    </Box>
                    <Chip 
                        icon={<TableIcon sx={{ color: '#1e3c72 !important' }} />}
                        label="clean_data.resource_detail_file" 
                        sx={{ 
                            bgcolor: 'white', 
                            color: '#1e3c72', 
                            fontWeight: 600,
            ...
            <Card sx={{ mb: 3, borderRadius: 2, boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
                <Box sx={{ p: 2.5 }}>
                    <Box sx={{ display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap' }}>
                        ...
                        <Button
                            variant={advancedFilterOpen ? "contained" : "outlined"}
                            onClick={toggleAdvancedFilter}
                            startIcon={<FilterIcon />}
                            endIcon={advancedFilterOpen ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                            sx={{ 
                                borderRadius: 2,
                                px: 3,
                                textTransform: 'none',
                        ...
                        >
                            ...
                            {Object.keys(filters).length > 0 && (
                                <Chip 
                                    ...
                                    sx={{ ml: 1, bgcolor: 'rgba(255,255,255,0.3)', height: 20, fontSize: '0.7rem' }} 
                                />
                            )}
                        </Button>
                    ...
                    <Menu
                        ...
                        PaperProps={{ sx: { maxHeight: 400, width: 280, borderRadius: 2 } }}
                    >
                        <Box sx={{ px: 2, py: 1, display: 'flex', gap: 1 }}>
                            <Button size="small" onClick={handleSelectAllColumns} sx={{ flex: 1, fontSize: '0.75rem' }}>
                                Tout
                            </Button>
                            <Button size="small" onClick={handleDeselectAllColumns} sx={{ flex: 1, fontSize: '0.75rem' }}>
                                Aucun
                            </Button>
                        </Box>
                        <Divider />
                        {availableColumns.map((column) => (
                            <MenuItem key={column} dense onClick={() => handleToggleColumn(column)}>
                                ...
                                <ListItemText 
                                    primary={tableData?.columns.find(c => c.name === column)?.label || column}
                    ...
                    <Collapse in={advancedFilterOpen}>
                        <Box sx={{ 
                            ...
                            <Typography variant="subtitle2" sx={{ mb: 2, fontWeight: 700, color: '#1e3c72', display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Dialog open={viewDialogOpen} onClose={handleViewDialogClose} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
                <DialogTitle sx={{ 
                    ...
                    <IconButton onClick={handleViewDialogClose} sx={{ color: 'white' }}>
                        <CloseIcon />
                    </IconButton>
                </DialogTitle>
            ...
            <Dialog open={editDialogOpen} onClose={handleEditDialogClose} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
                <DialogTitle sx={{ 
                    ...
                    <IconButton onClick={handleEditDialogClose} sx={{ color: 'white' }}>
                        <CloseIcon />
                    </IconButton>
                </DialogTitle>
                <DialogContent dividers sx={{ p: 3 }}>
                ...
                <DialogActions sx={{ p: 2, gap: 1 }}>
                    ...
                    <Button onClick={handleSaveEdit} variant="contained" sx={{ borderRadius: 2, px: 3 }}>
                        Enregistrer
                    </Button>
                </DialogActions>
            </Dialog>
            ...
            <Dialog open={deleteDialogOpen} onClose={handleDeleteDialogClose} PaperProps={{ sx: { borderRadius: 3 } }}>
                ...
                <DialogActions sx={{ p: 2, gap: 1 }}>
                    ...
                    <Button onClick={handleConfirmDelete} color="error" variant="contained" sx={{ borderRadius: 2, px: 3 }}>
                        Supprimer
                    </Button>
                </DialogActions>
            </Dialog>
...
export default ResourceDetailFilePageAdvanced;
