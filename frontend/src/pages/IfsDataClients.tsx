...
const IfsDataClients = () => {
  ...
  // Chargement initial des tables (utilisera l'endpoint spécifique aux clients)
  useEffect(() => {
    const loadTables = async () => {
      try {
        const tablesData = await ifsTablesService.getClientTables();
        setTables(tablesData);
        if (tablesData.length > 0) {
          setSelectedTable(tablesData[0].id);
        }
      } catch (error) {
        console.error('Erreur lors du chargement des tables clients:', error);
      }
    };
  ...
  const handleExport = async (format: 'csv' | 'excel') => {
    ...
    try {
      ...
      const options: ExportOptions = {
        ...
        searchTerm,
        filters,
        filename: `ifs_clients_${selectedTable.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().toISOString().split('T')[0]}`
      };
      
      const blob = await ifsTablesService.exportTableData(selectedTable, fields, options);
      ifsTablesService.downloadExport(blob, `${options.filename}.${format === 'excel' ? 'xlsx' : 'csv'}`);
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const recordId = currentRow.client_no || currentRow.id || Object.values(currentRow)[0];
      
      const result = await ifsTablesService.updateRecord(selectedTable, recordId, editedValues);
  ...
  const handleDeleteMultiple = async (rowsToDelete: any[]) => {
    ...
    try {
      ...
      const getRecordId = (row: any) => {
        if (selectedTable.includes('client') && row.client_no) {
          return row.client_no;
        } 
        else if (row.id) {
          return row.id;
        }
          ...
          if (idFields.length > 0) {
            const sortedFields = idFields.sort((a, b) => {
              ...
              if (a.toLowerCase() === 'client_no') return -1;
              if (b.toLowerCase() === 'client_no') return 1;
              return 0;
            });
