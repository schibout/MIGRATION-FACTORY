import { Inventory as ArticleIcon, Refresh as RefreshIcon } from '@mui/icons-material';
import {
    Alert,
    Box,
    Button,
...
const ExportArticles: React.FC = () => {
  ...
  useEffect(() => {
    const loadTables = async () => {
      try {
        ...
        const data = await loadExportQueries('inventory');
        setAvailableTables(data.queries);
        
        // Sélectionner quelques tables par défaut pour les articles
        const defaultTables = data.queries
          .filter(q => ['ARTICLE', 'ARTICLE_INFO', 'INVENTORY_PART'].includes(q.table_name))
          .map(q => q.table_name);
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      console.log('📋 Configuration d\'export articles:', exportConfig);
      
      // Appeler le service d'export via l'API backend (téléchargement automatique)
      const result = await exportArticlesData(exportConfig);
      
      console.log('✅ Export articles terminé avec succès:', result);
      ...
      console.error('❌ Erreur lors de l\'export articles:', error);
      setError(error.message || 'Erreur lors de l\'export des données articles');
    } finally {
      setExporting(false);
    }
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 800, mx: 'auto' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Exportez les données des articles et du catalogue dans le format de votre choix
            </Typography>
          </Box>
        </Box>
      ...
      <Alert severity="info" sx={{ mb: 3 }}>
        {availableTables.length} tables d'export articles disponibles (chargées dynamiquement depuis la base de données)
      </Alert>
...
// Force recompilation - temporary fix 