import {
    ...
    CloudDownload as SharePointIcon
...
const ImportProjetsPage: React.FC = () => {
    ...
    const testConnection = async () => {
        try {
            ...
            const response = await api.get('/import/projets/test-connection');
    ...
    const loadStatus = async () => {
        try {
            const response = await api.get('/import/projets/status');
    ...
    const handleStartImport = async () => {
        ...
        try {
            console.log('🚀 Import des projets SharePoint via le backend...');
            
            // Le backend fait tout : fetch SharePoint + save en DB
            // Aucune limite ni filtre de statut : import complet
            const response = await api.post('/import/projets', {});
            ...
            if (response.data.success) {
                ...
                if (dbSaved) {
                    console.log(`✅ Import terminé: ${response.data.imported_count} projets sauvegardés en DB`);
                } else {
                    console.log(`⚠️ Import terminé: ${response.data.imported_count} projets récupérés (non sauvegardés en DB)`);
                }
    ...
    return (
        <Container maxWidth="lg" sx={{ py: 4 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 4 }}>
                ...
                <SharePointIcon sx={{ fontSize: 40, color: '#2196f3', mr: 2 }} />
                <Box>
                    <Typography variant="h4" component="h1" sx={{ fontWeight: 600 }}>
                        Import Projets SharePoint
                    </Typography>
                    <Typography variant="body1" color="text.secondary">
                        Importez vos projets depuis SharePoint vers la base de données
                    </Typography>
                </Box>
            </Box>
                        ...
                        <Box>
                                ...
                                <Card sx={{ mb: 3 }}>
                                    <CardContent>
                                        ...
                                        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                                            <Chip
                                                label={`${status.total_projets} projets`}
                                                color="primary"
                                                variant="outlined"
                                            />
                                            <Chip
                            ...
                            <Card sx={{ mb: 3 }}>
                                <CardContent>
                                    ...
                                    <Typography variant="body2" color="text.secondary">
                                        Import complet : tous les projets SharePoint sont importés,
                                        sans limite de nombre et quel que soit leur statut
                                        (En cours, Clôturé, Annulé…).
                                    </Typography>
                                </CardContent>
                            </Card>
                            ...
                            <Box sx={{ textAlign: 'center' }}>
                                <Button
                                    variant="contained"
                                    size="large"
                                    startIcon={loading ? <CircularProgress size={20} /> : <SharePointIcon />}
                    ...
                    {activeStep === 1 && (
                        <Box sx={{ textAlign: 'center', py: 4 }}>
                            ...
                            <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
                                Récupération des projets depuis SharePoint
                            </Typography>
                            <Typography variant="body2" color="text.secondary">
                                Import complet — aucune limite, tous statuts
                            </Typography>
                        </Box>
                    )}
                    ...
                    {activeStep === 2 && importResult && (
                        <Box sx={{ textAlign: 'center', py: 4 }}>
                            ...
                            <Card sx={{ mb: 4, mx: 'auto', maxWidth: 500 }}>
                                <CardContent>
                                    ...
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                                        <Typography variant="body2">Projets importés:</Typography>
                                        <Typography variant="body2" sx={{ fontWeight: 600 }}>
                            ...
                            <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center' }}>
                                ...
                                <Button
                                    variant="contained"
                                    onClick={() => navigate('/import/sharepoint-projects')}
                                >
                                    Voir les Projets
                                </Button>
                            </Box>
...
export default ImportProjetsPage; 