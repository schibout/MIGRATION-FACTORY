...
const ImportRessourcesPage: React.FC = () => {
    ...
    const testConnection = async () => {
        try {
            ...
            const response = await api.get('/import/ressources/test-connection');
    ...
    const loadStatus = async () => {
        try {
            const response = await api.get('/import/ressources/status');
    ...
    const handleStartImport = async () => {
        ...
        try {
            console.log('🚀 Import des ressources SharePoint via le backend...');
            
            // Le backend fait tout : fetch SharePoint + save en DB
            const response = await api.post('/import/ressources', {
                limit: limitValue
            });
            ...
            if (response.data.success) {
                ...
                if (dbSaved) {
                    console.log(`✅ Import terminé: ${response.data.imported_count} ressources sauvegardées en DB`);
                } else {
                    console.log(`⚠️ Import terminé: ${response.data.imported_count} ressources récupérées (non sauvegardées en DB)`);
                }
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mb: 4 }}>
                ...
                <Box>
                    <Typography variant="h4" component="h1" sx={{ fontWeight: 600 }}>
                        Import Ressources SharePoint
                    </Typography>
                    <Typography variant="body1" color="text.secondary">
                        Importez vos ressources depuis SharePoint vers la base de données
                    </Typography>
                </Box>
            </Box>
                        ...
                        <Box>
                            ...
                            <Card sx={{ mb: 3 }}>
                                <CardContent>
                                    ...
                                    <Typography variant="body2" color="text.secondary">
                                        URL: http://asap.stjn.local/_api/web/lists/getByTitle('Ressources')/items
                                    </Typography>
                                </CardContent>
                            </Card>
                                ...
                                <Card sx={{ mb: 3 }}>
                                    <CardContent>
                                        ...
                                        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                                            <Chip
                                                label={`${status.total_resources} ressources`}
                                                color="primary"
                                                variant="outlined"
                                            />
                                            <Chip
                            ...
                            <Card sx={{ mb: 3 }}>
                                <CardContent>
                                    ...
                                    <TextField
                                        label="Limite de ressources"
                                        type="number"
                                        value={limitValue}
                                        onChange={(e) => setLimitValue(Number(e.target.value))}
                                        helperText="Nombre maximum de ressources à importer (recommandé: 100-1000)"
                                        sx={{ width: 300 }}
                    ...
                    {activeStep === 1 && (
                        <Box sx={{ textAlign: 'center', py: 4 }}>
                            ...
                            <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
                                Récupération des ressources depuis SharePoint
                            </Typography>
                            <Typography variant="body2" color="text.secondary">
                                Limite configurée: {limitValue} ressources
                            </Typography>
                        </Box>
                    )}
                            ...
                            <Card sx={{ mb: 4, mx: 'auto', maxWidth: 500 }}>
                                <CardContent>
                                    ...
                                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                                        <Typography variant="body2">Ressources importées:</Typography>
                                        <Typography variant="body2" sx={{ fontWeight: 600 }}>
...
export default ImportRessourcesPage;
